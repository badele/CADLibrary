...
def compare_images(a, b, c):
    if not os.path.isfile(b):
        print(
            Fore.MAGENTA + "Failed to generate %s while making %s" % (b, a), Fore.WHITE
        )
        sys.exit(1)
    ...
    with open(log_name, "w") as output:
        do_cmd(
            ("magick compare -metric AE -fuzz %d%% %s %s %s" % (fuzz, a, b, c)).split(),
            output=output,
        )
...
def update_image(tmp_name, png_name):
    ...
    if pixels < 0 or pixels > threshold:
        ...
        print(
            Fore.YELLOW + png_name + " updated" + Fore.WHITE,
            pixels if pixels > 0 else "",
        )
...
def tests(tests):
    ...
    scads = [
        i
        for i in sorted(os.listdir(scad_dir), key=lambda s: s.lower())
        if i[-5:] == ".scad"
    ]
    types = []
    for scad in scads:
        ...
        if not tests or base_name in tests:
            ...
            if libtest:
                ...
                vtype = locations[0][1]
                types = [
                    vtype + " " + vsplit[i] + "-" + chr(ord(vsplit[i + 1]) - 1)
                ...
                if not types:
                    ...
                    if (
                        not type in types
                    ):  # Will happen when implementation is not found and type is set to Tests
                        types.append(type)
            ...
            with open(bom_name, "rt") as bom_file:
                ...
                for thing, heading in [
                    ("vitamins", "Module call | BOM entry"),
                    ("printed", "Filename"),
                    ("routed", "Filename"),
                    ("assemblies", "Name"),
                ]:
